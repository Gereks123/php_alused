#German Eksi , 10.11.2020 Ülesanne 5
<?php

$nimed = ['kati', 'Mari', 'Mart', 'Mark'];
...
echo "<br>";
array_push($nimed, 'ahmed', 'Marie', 'Kalkun');
var_dump($nimed);
echo "<br>";
array_unshift($nimed, 'Kart'); #väärtus läheb massiivi algusesse.
var_dump($nimed);

$nimed[15] = 'Mees'; #Sellega saab määrata kindla indeksi väärtusele koheselt, olemasolevad indeksid kirjutatakse üle.
array_pop($nimed); #eemaldab viimase liikme massiivist.


asort($hinded); #sorteeritakse väärtuse järgi
echo "<br>";
ksort($hinded); #sorteeritakse võtme järgi.
...
echo "<h1>Siit hakkab Tüdrukute ÜL</h1>";
...
echo "<br>"; # esimesed 3 nime
echo "$tüdrukud[0] <br> $tüdrukud[1] <br> $tüdrukud[2]";
echo "<br>";
...
#Random nimi
echo "<br> <h5>Random nimi</h5>";
$rand_keys = array_rand($tüdrukud);
echo $tüdrukud[$rand_keys]. "<br>";
...
#Kogus
echo count($auto_mark). "--- Autode pikkus";
echo "<br>";
echo count($auto_VIN). "--- Vin pikkus";

#Toyota ja Audide count

$counts = array_count_values($auto_mark);
echo "<br>";
echo $counts['Toyota']; echo "----- Toyotade arv";

echo "<br>";
function vaartus($auto_VIN) {
    $len = 17;
    foreach($auto_VIN as $VIN) {
        if (strlen($VIN) < $len) {
            echo "$VIN <br>";
        }
    }
